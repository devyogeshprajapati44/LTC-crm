...
if($query->num_rows > 0){ 
    ...
    // Set column headers 
    $fields = array('ID', 'Employee Name', 'attendance_month', 'attendance_year','P(Present)','W(WeekOFF)','CO(Combo-Off)','H(Holiday)','GH(Gazetted Holiday)','HD(Half-Day)','L(Leave)','CreatedOn'); 
    fputcsv($f, $fields, $delimiter); 
     $id=1;
    // Output each row of the data, format line as csv and write to file pointer 
    while($row = $query->fetch_assoc()){ 
        //$status = ($row['status'] == 1)?'Active':'Inactive'; 
        $lineData = array($id++, $row['emp_name'], $row['attendance_month'], $row['attendance_year'], $row['P'], $row['WO'], $row['CO'],$row['H'],$row['GH'],$row['HD'],$row['L'],$row['CreatedOn']); 
