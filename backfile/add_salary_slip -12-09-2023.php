...
<section class="section">
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              <h5 class="card-title"><u><b>Salary List:-</b></u></h5>
              <!-- <form method="POST">
                          <a class="btn btn-warning text-black" href="" style="margin:-25;font-size:large;height:39px;padding:6px;width:100px;" role="button">Back</a>&nbsp;&nbsp;
                            ...
                            <input type="submit" class="btn btn-outline-success my-2 my-sm-0" name="submitsearch" style="margin:-25;height:41px;padding:6px;width:100px;" value="Search">
                              ...
                              <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#myModal" style="float:right;margin-top: -38px;" id="addsalary">+ Add Salary</button>
                              <br/><br/>
              <!-- Default Accordion -->
              <?php
                                        ...
                                        {
                                            $sql_year="SELECT `year` FROM `add_salary_slip` where `month`='".$_SESSION["Monthss"]."'";
                                            $run_y   = mysqli_query($conn,$sql_year);
                                            $row_year = mysqli_fetch_array($run_y);
                                            $yearss=$row_year["year"];
                                          ?> 
                            ...
                            <thead class="thead-pink">
                              <tr>
                                ...
                                <?php
                                  ...
                                  {
                                     $sql = "SELECT `asal`.*, `us`.`Id`,`us`.`emp_code`,`us`.`emp_name`,DATE_FORMAT(`us`.`joining_date`,'%m-%Y') AS `year_and_month`,concat(`desig`.`Designation`, ' / ' ,`depart`.`department`) as `Desig_n_Depart`FROM `add_salary_slip` `asal` LEFT JOIN `employee` `us` ON `asal`.`user_id`=`us`.`Id` LEFT JOIN `designation` `desig` ON `asal`.`desig_id`=`desig`.`desig_id` LEFT JOIN `department` `depart` ON `asal`.`dept_id`=`depart`.`dept_id` where `us`.`emp_status`='1' AND `month`='$monthNum' order by `us`.Id LIMIT $offset, $no_of_records_per_page";
                                  }
                                  ...
                                  if(isset($_POST['submitsearch']))
                                  {
                                    ...
                                    {
                                      $filtervalue=strtoupper($_REQUEST['searchvalue']);
                                      $sql = "SELECT `asal`.*, `us`.`Id`,`us`.`emp_code`,`us`.`emp_name`,DATE_FORMAT(`us`.`joining_date`,'%m-%Y') AS `year_and_month`,concat(`desig`.`Designation`, ' / ' ,`depart`.`department`) as `Desig_n_Depart`FROM `add_salary_slip` `asal` LEFT JOIN `employee` `us` ON `asal`.`user_id`=`us`.`Id` LEFT JOIN `designation` `desig` ON `asal`.`desig_id`=`desig`.`desig_id` LEFT JOIN `department` `depart` ON `asal`.`dept_id`=`depart`.`dept_id` where `us`.`emp_status`='1' AND `us`.`Id`='$pfc_uid' AND `us`.`emp_name` LIKE '%$filtervalue' OR `us`.`emp_name` LIKE '$filtervalue%' OR `us`.`emp_name` LIKE '%$filtervalue%' AND `month`='$monthNum' order by `us`.Id desc LIMIT $offset, $no_of_records_per_page";
                                    }
                                  }
