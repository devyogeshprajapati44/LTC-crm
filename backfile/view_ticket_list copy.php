      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <nav class="navbar navbar-light bg-light">
                  <a class="btn btn-warning text-black" href="PFC.php?PageName=view_ticket_list" style="margin:7px;font-size:large;height:37px;padding:6px;width:100px;" role="button">Back</a>&nbsp;&nbsp;
                    <form method="POST">
            ...
            <div class="card-body">
              <h5 class="card-title"><u>View Ticket Details:-</u></h5>
                ...
                <tbody>
                  <?php
                            ...
                            {
                                $sql="SELECT `ts`.`Id`, `ts`.`user_id`, `ts`.`ticket_no`, `ts`.`type_of_ticket`, `ts`.`complain_issue`, `ts`.`status`, `ts`.`ticket_close_date`, `ts`.`CreatedOn`,`emp`.`Id`,`emp`.`emp_name` FROM `ticket_system` `ts` LEFT JOIN `employee` `emp` ON `ts`.`user_id`=`emp`.`Id`"."$limit";
                            }

                            if(isset($_POST['submitsearch']))
                            {
                               $filtervalue=strtoupper($_REQUEST['searchvalue']);
                               $sql="SELECT `ts`.`Id`, `ts`.`user_id`, `ts`.`ticket_no`, `ts`.`type_of_ticket`, `ts`.`complain_issue`, `ts`.`status`, `ts`.`ticket_close_date`, `ts`.`CreatedOn`,`emp`.`Id`,`emp`.`emp_name` FROM `ticket_system` `ts` LEFT JOIN `employee` `emp` ON `ts`.`user_id`=`emp`.`Id` WHERE `emp`.`emp_name` LIKE '%$filtervalue' OR `emp`.`emp_name` LIKE '$filtervalue%' OR `emp`.`emp_name` LIKE '%$filtervalue%'"."$limit";
                           }
                           if(($_SESSION['PFC_EmpRole']!=1) || ($_SESSION['PFC_EmpRole']!=5))
                           {
                                $sql="SELECT `ts`.`Id` as `idd`, `ts`.`user_id`, `ts`.`ticket_no`, `ts`.`type_of_ticket`, `ts`.`complain_issue`, `ts`.`status`, `ts`.`ticket_close_date`, `ts`.`CreatedOn`,`emp`.`Id`,`emp`.`emp_name` FROM `ticket_system` `ts` LEFT JOIN `employee` `emp` ON `ts`.`user_id`=`emp`.`Id`"."$condition"."$limit";
                           }
