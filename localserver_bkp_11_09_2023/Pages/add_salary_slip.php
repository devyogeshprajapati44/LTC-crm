...
<!--Data table--->
                  <!-- <nav class="navbar navbar-light bg-light">
                      <form method="POST">
                        ...
                        <!--table start-->
                        <div class="card-body">
                            <table id="myTabless11" class="table table-striped" style="width:100%">
                            <thead class="thead-pink">
                              <tr>
                                ...
                                <?php
                                  ...
                                  {
                                     $sql = "SELECT `asal`.*, `us`.`Id`,`us`.`emp_code`,`us`.`emp_name`,CONCAT(DATE_FORMAT(CONCAT(`asal`.`year`, '-', `asal`.`month`, '-01'), '%M'), ' ', year) AS `month_and_year` ,concat(`desig`.`Designation`, ' / ' ,`depart`.`department`) as `Desig_n_Depart`FROM `add_salary_slip` `asal` LEFT JOIN `employee` `us` ON `asal`.`user_id`=`us`.`Id` LEFT JOIN `designation` `desig` ON `asal`.`desig_id`=`desig`.`desig_id` LEFT JOIN `department` `depart` ON `asal`.`dept_id`=`depart`.`dept_id` where `us`.`emp_status`='1' AND `month`='$monthNum' order by `us`.Id";
                                  }
                                  ...
                                  if(isset($_POST['submitsearch']))
                                  {
                                    if(($_SESSION['PFC_EmpRole']!=1) || ($_SESSION['PFC_EmpRole']!=5))
                                    {
                                      ...
                                      $sql = "SELECT `us`.`Id`,`us`.`emp_code`,`us`.`emp_name`,CONCAT(DATE_FORMAT(CONCAT(`asal`.`year`, '-', `asal`.`month`, '-01'), '%M'), ' ', year) AS `month_and_year`,concat(`desig`.`Designation`, ' / ' ,`depart`.`department`) as `Desig_n_Depart` FROM `employee` `us` LEFT JOIN `designation` `desig` ON `us`.`desig_id`=`desig`.`desig_id` LEFT JOIN `department` `depart` ON `us`.`dept_id`=`depart`.`dept_id` where `us`.`emp_status`='1' AND `us`.`Id`='$pfc_uid' AND `us`.`emp_name` LIKE '%$filtervalue%' AND `month`='$monthNum' order by `us`.Id desc";
                                    }
                                    if(($_SESSION['PFC_EmpRole']==1) || ($_SESSION['PFC_EmpRole']==5))
                                    {
                                      $filtervalue=strtoupper($_REQUEST['searchvalue']);
 ...
 <br>                                                                        
  ...
  <?php
        ...
        if(isset($_POST["empView"]))
        {
          $id=$_REQUEST["empView"]; 
          $EID=EDV($id,1);
          header("location:PFC.php?PageName=print_payslip&EmpID=".$EID);
        }
?>  
<script>
   function addsalary1()
